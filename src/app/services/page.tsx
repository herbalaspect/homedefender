...
const HUB_HERO_IMAGE = "/images/hero_services-hero.webp";
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {serviceCategories.map((c) => (
              <Link
                key={c.slug}
                href={c.path}
                className="group block overflow-hidden rounded-img border border-gray-200 bg-white transition-shadow hover:shadow-md"
              >
                {CATEGORY_CARD_IMAGE[c.slug] && (
                  <div className="relative aspect-[16/10] w-full overflow-hidden">
                    <Image
                      src={CATEGORY_CARD_IMAGE[c.slug]}
                      alt={`${c.h1} in Madison and Dane County`}
                      fill
